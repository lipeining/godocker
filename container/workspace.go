...
// 创建容器运行时目录
func NewWorkSpace(c *Container, imageName string) error {
	// 1. 创建只读层
...
func createReadOnlyLayer(imageName string) error {
	...
	imageTarPath := path.Join(RootPath, fmt.Sprintf("%s.tar", imageName))
...
func CreateMountPoint(containerName, imageName string) error {
	mntPath := path.Join(MntPath, containerName)
	...
	writeLayPath := path.Join(RootPath, WriteLayer, containerName)
	imagePath := path.Join(RootPath, imageName)
...
func mountVolume(containerName, imageName, volume string) {
	if volume != "" {
		...
		if len(volumes) > 1 {
			...
			containerVolumePath := path.Join(MntPath, containerName, containerPath)
...
func deleteVolume(containerName, volume string) {
	if volume != "" {
		volumes := strings.Split(volume, ":")
		if len(volumes) > 1 {
			mntPath := path.Join(MntPath, WriteLayer, containerName)
